...
The shared scheme (`StandupBuddy.xcodeproj/xcshareddata/xcschemes/StandupBuddy.xcscheme`) **is** committed, but `xcodegen generate` deletes it. Restore it afterward with `git checkout -- StandupBuddy.xcodeproj/xcshareddata/xcschemes/StandupBuddy.xcscheme` (or re-share it from Xcode). Its build-target identifiers are stable, so the committed scheme works against a freshly generated project.
